...
def add_table(ws, ref: str, display_name: str) -> None:
    """
    Agrega una tabla de Excel solo cuando el rango contiene
    encabezados y al menos una fila adicional.

    Esto evita que Excel repare y elimine una tabla vacía.
    """
...
def create_master_workbook(path: Path, max_rows: int = 500) -> None:
    ...
    # La hoja maestra inicia vacía.
    # Se aplica un filtro normal sin crear una Tabla de Excel vacía.
    last_master_column = ws.cell(
        row=1,
        column=len(MASTER_COLUMNS)
    ).coordinate

    ws.auto_filter.ref = f"A1:{last_master_column}"
    wb.save(path)
